...
function point_to_hex(point: PointG1): string {
...
describe("sync-commitments", function () {
    ...
    test_cases.push([
        [point_to_bigint(refpubkeys[0]), point_to_hex(refpubkeys[0])],
        [point_to_bigint(refpubkeys[1]), point_to_hex(refpubkeys[1])],
        [point_to_bigint(refpubkeys[2]), point_to_hex(refpubkeys[2])],
        [point_to_bigint(refpubkeys[3]), point_to_hex(refpubkeys[3])],
    ]);
    ...
    var test_poseidon_commitment = function (
        ...
        let [pub0x, pub0y] = test_case[0][0];
        let [pub1x, pub1y] = test_case[1][0];
        let [pub2x, pub2y] = test_case[2][0];
        let [pub3x, pub3y] = test_case[3][0];

        let pub0hex = test_case[0][1];
        let pub1hex = test_case[1][1];
        let pub2hex = test_case[2][1];
        let pub3hex = test_case[3][1];
        ...
        let pubkeysHex = [
            hexToBytes(pub0hex), hexToBytes(pub1hex), hexToBytes(pub2hex), hexToBytes(pub3hex)
        ]

        it("sync-commitment", async function () {

            let refhash = await poseidon_aggregation(pubkeysInt.flat().flat());
